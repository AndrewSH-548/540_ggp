...
{	
	...
	{
		for (int i = 0; i < entities.size(); i++) {
			ConstructShaderData(entities[i].GetTransform()->GetWorldMatrix());
			entities[i].Draw();
		}
...
void Game::ConstructShaderData(XMFLOAT4X4 worldMatrix) {
	// Set shader effect
...
void Game::BuildUI() {
	...
	for (int i = 0; i < entities.size(); i++) {
		...
		ImGui::SliderFloat3("Rotation (Radians)", &rotation.x, -3.14, 3.14);
